...
export default function App() {
  ...
  const totalHoursSaved = history.reduce((sum, item) => sum + getHoursSaved(item.risk), 0).toFixed(1);
  const totalCostSaved = history.reduce((sum, item) => sum + getCostSaved(item.risk), 0);
  ...
  const createAccount = () => {
    ...
    localStorage.setItem("mdr_account", JSON.stringify({ email: email.trim(), password: password.trim() }));
  ...
  const uploadAndAnalyze = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:8000/analyze/${uploadData.document_id}`);
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(11);
    doc.text(doc.splitTextToSize(result.summary_one_liner || "No summary available.", 170), 20, 65);
    ...
    doc.text(`Estimated review effort: ${getReviewEffort(riskScore)}`, 20, 156);
    doc.text(`Estimated hours saved: ${getHoursSaved(riskScore)}h`, 20, 164);
    doc.text(`Estimated QA cost saved: €${getCostSaved(riskScore)}`, 20, 172);

    doc.setFontSize(14);
    doc.text("Priority Findings", 20, 192);
    doc.setFontSize(11);
    (result.priority_findings || []).slice(0, 5).forEach((item, i) => {
      doc.text(doc.splitTextToSize(`- ${item}`, 160), 25, 202 + i * 10);
    });
...
`;
  ...
  if (!isLoggedIn) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f8fafc", fontFamily: "Arial" }}>
        <div style={{ ...card, width: 390 }}>
          ...
          <button onClick={authMode === "login" ? login : createAccount} style={{ width: "100%", padding: 12 }}>
            {authMode === "login" ? "Login" : "Create account"}
          </button>

          <p style={{ marginTop: 20 }}>
            {authMode === "login" ? "No account?" : "Already have an account?"}{" "}
            <span onClick={() => setAuthMode(authMode === "login" ? "register" : "login")} style={{ color: "#2563eb", cursor: "pointer" }}>
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f8fafc", fontFamily: "Arial" }}>
      ...
      <main style={{ flex: 1, padding: 40 }}>
        ...
        {activePage === "dashboard" && (
          <>
            <section style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 20, marginBottom: 25, marginTop: 25 }}>
              ...
              <div style={card}><h3>Hours saved</h3><h1>{totalHoursSaved}h</h1></div>
              <div style={card}><h3>QA cost saved</h3><h1>€{totalCostSaved}</h1></div>
            </section>
            ...
            {result && (
              <section style={{ ...card, marginTop: 20 }}>
                ...
                <p>Risk: <b>{result.risk_score}/10</b></p>
                <p>Review effort: <b>{getReviewEffort(result.risk_score)}</b></p>
        ...
        {activePage === "documents" && (
          <section style={card}>
            <h2>Documents</h2>
            <input placeholder="Search" value={search} onChange={(e) => setSearch(e.target.value)} style={{ padding: 10, marginBottom: 15, width: "100%" }} />

            {filteredHistory.map((item, i) => (
              <div key={i} style={{ padding: 16, border: "1px solid #eee", borderRadius: 12, marginTop: 10, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                onClick={() => {
        ...
        {activePage === "reports" && (
          <section style={card}>
            <h2>Reports</h2>
            <button onClick={downloadPDF} style={button}>Download Compliance Report</button>
        ...
        {activePage === "settings" && (
          <section style={card}>
            <h2>Settings</h2>
            <button onClick={() => { localStorage.removeItem(historyKey); setHistory([]); }} style={button}>Clear history</button>
            <button onClick={logout} style={{ ...button, marginLeft: 10, background: "#ef4444" }}>Logout</button>
          </section>
        )}
      </main>
    </div>
